...
class Realtimeplot(object):
    ...
    def __init__(self, sampleinterval=0.1, timewindow=50.):
        ...
        self.y_dsp = collections.deque(np.zeros(self._bufsize), self._bufsize)
        self.x_dsp = np.arange(self._bufsize)         
    ...
    def updateplot_dsp(self, data):
        self.y_dsp.extend(data)
        self.curve_dsp.setData(self.x_dsp, self.y_dsp)        
...
class Interface_Graphique(QtGui.QWidget):
    ...
    def __init__(self):
        ...
        self.plot_set_dsp = Realtimeplot()
        self._active = False
	   ...
	   ################################################################
        ...
        grid.addWidget(self.plot_set.canvas_plot, 3, 0)
        grid.addWidget(self.button_panel, 3, 2)
        grid.addWidget(self.button_quit, 4,1)
        grid.addWidget(self.plot_set_dsp.canvas_plot_dsp, 3, 1)        
    ...
    def freerun(self):
        ...
        while self._active:
            ...
            data_dsp = self.carte.dsp(data) ### Ajout d'une dsp
            self.plot_set.updateplot(data)
            self.plot_set_dsp.updateplot_dsp(data_dsp)
